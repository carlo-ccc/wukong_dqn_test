        ...
        if done:
            ...
            if action_name == "DRINK_POTION":
                if b_status["self_blood"] > 70:
                    # 惩罚 满血 喝药
                    reward -= 100
                elif b_status["self_blood"] < 60:
                    # 奖励 血量低时 喝药
                    reward += 30
                elif b_status["self_blood"] < 40:
                    # 奖励 血量低时 喝药
                    reward += 50
                elif b_status["hulu"] < 8:
                    # 喝光了 还在喝
                    reward -= 100
                ...
                if not injured:
                    # 闪避 且没挨打
                    reward += 12
                    # 新增 gunshiline 状态逻辑优化
                    current_gunshi = {a_status["gunshi1"], a_status["gunshi2"], a_status["gunshi3"],
                                      a_status["gunshiline"]}
                    previous_gunshi = {b_status["gunshi1"], b_status["gunshi2"], b_status["gunshi3"]}

                    if a_status["gunshiline"] or (current_gunshi > previous_gunshi):
                        log.debug(
                            (
                                "ssssssssssssssshanshsnashans"
                            )
                        )
                        reward += 100
                else:
